...
class VatsView(BaseView):
    ...
    def copy(self):
        vat = Vat.get_copy(self.request.params.get('id'))
        return {
    ...
    def _delete(self):
        ...
        if ids:
            try:
                items = DBSession.query(Vat).filter(
                    Vat.id.in_(ids)
                )
                for item in items:
    ...
    @view_config(
        name='assign',
        request_method='GET',
        renderer='travelcrm:templates/vats/assign.mako',
        permission='assign'
    )
    def assign(self):
        return {
    ...
    def _assign(self):
        form = VatAssignForm(self.request)
        if form.validate():
    ...
    @view_config(
        name='subscribe',
        request_method='GET',
        renderer='travelcrm:templates/vats/subscribe.mako',
        permission='view'
    )
    def subscribe(self):
        return {
    ...
    def _subscribe(self):
        ...
        for id in ids:
            vat = Vat.get(id)
            subscribe_resource(self.request, vat.resource)
        return {
