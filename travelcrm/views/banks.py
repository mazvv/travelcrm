...
class BanksView(BaseView):
    ...
    def _add(self):
        ...
        if form.validate():
            ...
            DBSession.add(bank)
            DBSession.flush()
            event = ResourceCreated(self.request, bank)
            event.registry()
            return {
    ...
    def _edit(self):
        ...
        if form.validate():
            form.submit(bank)
            event = ResourceChanged(self.request, bank)
            event.registry()
            return {
                'success_message': _(u'Saved'),
                'response': bank.id
    ...
    def copy(self):
        bank = Bank.get_copy(self.request.params.get('id'))
        return {
    ...
    def _delete(self):
        ...
        if ids:
            try:
                items = DBSession.query(Bank).filter(
                    Bank.id.in_(ids)
                )
                for item in items:
    ...
    @view_config(
        name='assign',
        request_method='GET',
        renderer='travelcrm:templates/banks/assign.mako',
        permission='assign'
    )
    def assign(self):
        return {
    ...
    def _assign(self):
        form = BankAssignForm(self.request)
        if form.validate():
    ...
    @view_config(
        name='subscribe',
        request_method='GET',
        renderer='travelcrm:templates/banks/subscribe.mako',
        permission='view'
    )
    def subscribe(self):
        return {
    ...
    def _subscribe(self):
        ...
        for id in ids:
            bank = Bank.get(id)
            subscribe_resource(self.request, bank.resource)
        return {
