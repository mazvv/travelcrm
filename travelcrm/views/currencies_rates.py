...
class CurrenciesRatesView(BaseView):
    ...
    def _add(self):
        ...
        if form.validate():
            ...
            DBSession.flush()
            event = ResourceCreated(self.request, currency_rate)
            event.registry()
            return {
    ...
    def _edit(self):
        ...
        if form.validate():
            ...
            event = ResourceChanged(self.request, currency_rate)
            event.registry()
            return {
                'success_message': _(u'Saved'),
                'response': currency_rate.id
    ...
    @view_config(
        name='subscribe',
        request_method='GET',
        renderer='travelcrm:templates/currencies_rates/subscribe.mako',
        permission='view'
    )
    def subscribe(self):
        return {
    ...
    def _subscribe(self):
        ...
        for id in ids:
            ...
            subscribe_resource(self.request, currency_rate.resource)
        return {
