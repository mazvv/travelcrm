...
class LeadsStatsView(BaseView):
    ...
    @view_config(
        request_method='GET',
        renderer='travelcrm:templates/leads_stats/index.mako',
        permission='view'
    )
    def index(self):
        return {
    ...
    def list(self):
        form = LeadsStatsSearchForm(self.request, LeadsResource(self.request))
        form.validate()
        qb = form.submit()
        labels = [
        ...
        data = [0] * len(labels)
        for row in qb.query:
            index = labels.index(row.lead_date)
            data[index] = row.quan
    ...
    @view_config(
        name='settings',
        request_method='GET',
        renderer='travelcrm:templates/leads_stats/settings.mako',
        permission='settings',
    )
    def settings(self):
    ...
    def _settings(self):
        form = LeadsStatsSettingsForm(self.request)
        if form.validate():
            form.submit()
