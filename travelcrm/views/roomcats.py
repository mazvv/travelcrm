...
class RoomcatsView(BaseView):
    ...
    def _add(self):
        ...
        if form.validate():
            ...
            DBSession.flush()
            event = ResourceCreated(self.request, roomcat)
            event.registry()
            return {
    ...
    def _edit(self):
        ...
        if form.validate():
            form.submit(roomcat)
            event = ResourceChanged(self.request, roomcat)
            event.registry()
            return {
                'success_message': _(u'Saved'),
                'response': roomcat.id
    ...
    @view_config(
        name='assign',
        request_method='GET',
        renderer='travelcrm:templates/roomcats/assign.mako',
        permission='assign'
    )
    def assign(self):
        return {
    ...
    def _assign(self):
        form = RoomcatAssignForm(self.request)
        if form.validate():
