...
class HotelsView(BaseView):
    ...
    @view_config(
        request_method='GET',
        renderer='travelcrm:templates/hotels/index.mako',
        permission='view'
    )
    def index(self):
        return {
    ...
    @view_config(
        name='view',
        request_method='GET',
        renderer='travelcrm:templates/hotels/form.mako',
        permission='view'
    )
    def view(self):
    ...
    def _add(self):
        ...
        if form.validate():
            ...
            DBSession.add(hotel)
            DBSession.flush()
            event = ResourceCreated(self.request, hotel)
            event.registry()
            return {
    ...
    def _edit(self):
        ...
        if form.validate():
            form.submit(hotel)
            event = ResourceChanged(self.request, hotel)
            event.registry()
            return {
                'success_message': _(u'Saved'),
                'response': hotel.id
    ...
    @view_config(
        name='delete',
        request_method='GET',
        renderer='travelcrm:templates/hotels/delete.mako',
        permission='delete'
    )
    def delete(self):
        return {
    ...
    def _delete(self):
        ...
        if ids:
            try:
                items = DBSession.query(Hotel).filter(
                    Hotel.id.in_(ids)
                )
                for item in items:
    ...
    @view_config(
        name='subscribe',
        request_method='GET',
        renderer='travelcrm:templates/hotels/subscribe.mako',
        permission='view'
    )
    def subscribe(self):
        return {
    ...
    def _subscribe(self):
        ...
        for id in ids:
            hotel = Hotel.get(id)
            subscribe_resource(self.request, hotel.resource)
        return {
