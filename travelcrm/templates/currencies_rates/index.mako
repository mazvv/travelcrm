...
<div class="easyui-panel unselectable"
    ...
    <div class="datagrid-toolbar" id="${_tb_id}">
        ...
        <div class="ml50 tr">
            <div class="search">
                ${searchbar(_id, _s_id, prompt=_(u'Enter currency code'))}
                <div class="advanced-search tl hidden" id = "${_s_id}">
                    <div>
                        ${h.tags.title(_(u"currency"))}
                    </div>
                    <div>
                        ${h.fields.currencies_combogrid_field(request, 'currency_id', show_toolbar=False)}
                    </div>
                    <div class="mt05">
                        ${h.tags.title(_(u"supplier"))}
                    </div>
                    <div>
                        ${h.fields.suppliers_combogrid_field(request, 'supplier_id', show_toolbar=False)}
                    </div>
                    <div class="mt05">
                        ${h.tags.title(_(u"rate date"))}
                    </div>
                    <div>
                        ${h.fields.date_field('rate_from')}
                        <span class="p1">-</span>
                        ${h.fields.date_field('rate_to')}
                    </div>
                    <div class="mt05">
