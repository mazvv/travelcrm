<%def name="subaccounts_selector(name='subaccount_id', values=[], can_edit=True)">
    <%
    ...
    <table class="easyui-datagrid"
        id="${_id}"
        data-options="
            url:'/subaccounts/list',border:false,
            singleSelect:true,
        ...
        <thead>
            ...
            <th data-options="field:'subaccount_type',sortable:true,width:100">${_(u"subaccount type")}</th>
            <th data-options="field:'currency',sortable:true,width:60">${_(u"currency")}</th>
        </thead>
    </table>
    <div id="${_storage_id}">
        % for subaccount in values:
            ${h.tags.hidden(name, subaccount)}
        % endfor
    </div>
    % if can_edit:
    <div class="datagrid-toolbar" id="${_tb_id}">
        <div class="actions button-container dl40">
            <div style="display: inline-block;padding-top:2px;">
                ...
                %>
                ${h.fields.subaccounts_combobox_field(request, None, f_id, id=f_id)}
            </div>
            <div class="button-group minor-group ml1">
                <a href="#" class="button" onclick="add_${_func_id}('${f_id}');">${_(u"Add")}</a>
                % if 'edit' in h.permisions.get_subaccounts_permisions(request):
                <a href="#" class="button _action" 
                    data-options="
                        container:'#${_id}',action:'dialog_open',url:'/subaccounts/edit',property:'with_row'
                    ">
                    ${_(u"Edit")}</a>
                % endif
